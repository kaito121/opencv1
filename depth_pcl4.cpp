...
std::cout<<"test1"<<std::endl;
   ...
   for(int x=0;x<RGBimage.size().width;x=x+W){
    ...
    int j=0;
    double MAXPX,MINPX,MAXPY,MINPY,PZ,leftz,rightz,centerz,centerx,centerleftx,centerleftz,centerrightx,centerrightz;
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = indices.begin (); it != indices.end (); ++it){ 
        ...
        for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit){ // グループ中の点にアクセスするループ(pit=グループ内の点番号)
            ...
            if(MINPY>=(pointCloud -> points[*pit].y * Y_wariai)){MINPY = pointCloud -> points[*pit].y * Y_wariai;}

            pointCloud -> points[*pit].x = ((pointCloud -> points[*pit].x*X_wariai)-(RGBimage.size().width/2))/X_pcl; //ピクセル座標からpcl上のX座標
            pointCloud -> points[*pit].y = ((pointCloud -> points[*pit].y*X_wariai)-(RGBimage.size().height/2))/Y_pcl;//ピクセル座標からpcl上のY座標
    ...
    line(img_dst9,Point(RGBimage.size().width/2,0),Point(RGBimage.size().width/2,RGBimage.size().height),Scalar(255,0,255),2);//画像の中心線
