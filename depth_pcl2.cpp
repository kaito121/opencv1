...
std::cout <<"["<<x<<"]["<<y<<"]="<<depthimage.at<float>(y,x)<< std::endl;//printf
   ...
   for(int x=0;x<RGBimage.size().width;x=x+W){
    ...
    int j=0;
    double MAXPX,MINPX,MAXPY,MINPY,PZ,leftz,rightz,centerz;
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = indices.begin (); it != indices.end (); ++it){ // グループにアクセスするループ(it=グループ番号)
        ...
        for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit){ // グループ中の点にアクセスするループ(pit=グループ内の点番号)
          ...
          // pointCloud -> points[*pit].z;
            pointCloud -> points[*pit].r=R[j%12];//ポイントクラウドのX座標
            pointCloud -> points[*pit].g=G[j%12];//ポイントクラウドのY座標
            ...
            if(MAXPX<=(pointCloud -> points[*pit].x*X_Y_wariai)){MAXPX = pointCloud -> points[*pit].x*X_Y_wariai;
                                                                                 rightz=pointCloud -> points[*pit].z;}
            if(MINPX>=(pointCloud -> points[*pit].x*X_Y_wariai)){MINPX = pointCloud -> points[*pit].x*X_Y_wariai;
                                                                                  leftz=pointCloud -> points[*pit].z;}
                                                        
            if(MAXPY<=(pointCloud -> points[*pit].y*X_Y_wariai)){MAXPY=pointCloud -> points[*pit].y*X_Y_wariai;}
            if(MINPY>=(pointCloud -> points[*pit].y*X_Y_wariai)){MINPY=pointCloud -> points[*pit].y*X_Y_wariai;}
            
            //if(MAX2<=(p/1000)){Dmax=p/1000;}
            //if(MINP2>=(p/1000)){Dmin=p/1000;}

            pointCloud -> points[*pit].x=((pointCloud -> points[*pit].x*X_Y_wariai)-(RGBimage.size().width/2))/X_Y_pcl; //pcl上のX座標
            pointCloud -> points[*pit].y=((pointCloud -> points[*pit].y*X_Y_wariai)-(RGBimage.size().height/2))/X_Y_pcl;//pcl上のY座標
            pointCloud -> points[*pit].z*=Z_pcl;

        }
    //itの回数が大枠の個数
    rectangle(img_dst9,Rect(MINPX,MINPY,MAXPX-MINPX+W,MAXPY-MINPY+H),Scalar(24,248,159),1.5);//四角作成
    putText(img_dst9,"No."+std::to_string(j),Point(MINPX,MINPY+30),0,0.5,Scalar(0,255,255),1);
    putText(img_dst9,"Left_Z="+std::to_string(leftz),Point(MINPX,MINPY+45),0,0.5,Scalar(0,255,255),1);
    putText(img_dst9,"right_z="+std::to_string(rightz),Point(MINPX,MINPY+60),0,0.5,Scalar(0,255,255),1);
    OpenCV1::waku waku;
    waku.point_left.x=(MINPX-(RGBimage.size().width/2))/X_Y_pcl;//送るデータ作成
    waku.point_left.y=(MAXPY-(RGBimage.size().width/2))/X_Y_pcl;
    waku.point_left.z=leftz;
    waku.point_right.x=(MAXPX-(RGBimage.size().width/2))/X_Y_pcl;
    waku.point_right.y=(MAXPY-(RGBimage.size().width/2))/X_Y_pcl;
