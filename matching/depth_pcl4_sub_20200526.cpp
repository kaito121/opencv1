...
int main(int argc,char **argv){//int argc=引数の個数 char **argv=引数の配列
	...
	message_filters::Subscriber<OpenCV1::wakuhairetu> waku_sub(nhSub, "wakuhairetu", 1);//認識データ受取
	message_filters::Subscriber<sensor_msgs::LaserScan> laser_sub(nhSub, "/robot1/scan", 1);//レーザデータ受取
