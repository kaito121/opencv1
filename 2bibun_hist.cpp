...
{
string name = "59232";
string file = "hist/59232";
string file_src = "image/"+name+".jpg";//元画像
string file_grey = "image3/"+file+"/B4_"+name+"grey.jpg";//グレースケール
string file_hist = "image3/"+file+"/B4_"+name+"hist.jpg";//ヒストグラム均一化
string file_cont = "image3/"+file+"/B4_"+name+"hist_cont.jpg";//コントラスト
//string file_meido = "image2/"+file+"/DSC_"+name+"hist_contup_meido0.jpg";//明度調整
string file_dst = "image3/"+file+"/B4_"+name+"hist_cont_after2.jpg";//二値化
string file_dst2 = "image3/"+file+"/B4_"+name+"hist_cont_after2_2tika.jpg";//二値化エッジ抽出画像
string file_dst3= "image3/"+file+"/B4_"+name+"hist_cont_after2_2tika_mask.jpg";//mask画像
string file_dst4 = "image3/"+file+"/B4_"+name+"hist_cont_after2_2tika_out.jpg";//カラーフィルタ処理画像
string file_dst5 = "image3/"+file+"/B4_"+name+"hist_cont_after2_2tika_out_UP.jpg";//膨張処理画像
string file_dst6 = "image3/"+file+"/B4_"+name+"hist_cont_after2_2tika_out_UP_mono.jpg";//密度検出画像
string file_dst7 = "image3/"+file+"/B4_"+name+"hist_cont_after2_2tika_out_UP_mono_mitudo.jpg";//密度検出画像
...
int W2=30,H2=30;
   for(i=0;i<=img_src.size().width-W2;i=i+W2){
	 for(j=0;j<=img_src.size().height-H2;j=j+H2){ 
		 siro=0; std::cout<<"test2"<<std::endl;
		 for(x=i;x<=i+W2;x=x+3){
			 for(y=j;y<=j+H2;y=y+3){
	            unsigned char intensity = img_dst5.at<unsigned char>(y, x);//白か黒かを調べる
	            std::cout <<"i:"<<i<<"j:"<<j<<"X:"<<x<<"y:"<<y<<"a:"<<(unsigned int)intensity<< std::endl;//表示
	            if(intensity>=200){siro=siro+1;} 
                 }}
