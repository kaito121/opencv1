...
std::cout<<"test"<<std::endl;
    ...
    int W=WINDOW_SIZE,H=WINDOW_SIZE;//マーカーの大きさ
...
Mat RGBhalf_img(img_dst7,Rect(0,RGBimage.size().height/2,RGBimage.size().width,RGBimage.size().height/2));
Mat DEPTHhalf_img(depthimage,Rect(0,RGBimage.size().height/2,RGBimage.size().width,RGBimage.size().height/2));

//白の割合を調べる
    for(int x = 0;x < RGBimage.size().width-W;x=x+W){
        for(int y = 0;y < RGBimage.size().height/2-H;y=y+H){
            Mat cut_img(RGBhalf_img,Rect(x,y,W,H)); // W*Hpx四方で切り抜く
            Mat cutdepth_img(DEPTHhalf_img,Rect(x,y,W,H)); // W*Hpx四方で切り抜く
   ...
   for(int x=0;x<RGBimage.size().width;x=x+W){
    ...
    for (std::vector<pcl::PointIndices>::const_iterator it = indices.begin (); it != indices.end (); ++it){ 
        ...
        for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit){ // グループ中の点にアクセスするループ(pit=グループ内の点番号)
            ...
            if(MAXPY<=(pointCloud -> points[*pit].y * Y_wariai)){MAXPY = pointCloud -> points[*pit].y * Y_wariai;}//yの最大値
            if(MINPY>=(pointCloud -> points[*pit].y * Y_wariai)){MINPY = pointCloud -> points[*pit].y * Y_wariai;}//X_Y_wariaiでかけるとピクセル値になる
            CENTPY=(MAXPY+MINPY)/2;

            pointCloud -> points[*pit].x = ((pointCloud -> points[*pit].x*X_wariai)-(RGBimage.size().width/2))/X_pcl; //ピクセル座標からpcl上のX座標
            pointCloud -> points[*pit].y = ((pointCloud -> points[*pit].y*Y_wariai)-(RGBimage.size().height/2))/Y_pcl;//ピクセル座標からpcl上のY座標
    ...
    //
        double CENPX,CENPXMAX,CENPXMIN,cenpx=1000,cenpxmin=1000,cenpxmax=1000;
         MINPX = pointCloud -> points[*(it->indices.end ()-1)].x * X_pcl +(RGBimage.size().width/2);//並び替えたので最大の値がXの最小値となる(leftX)
         MAXPX = pointCloud -> points[*(it->indices.begin ())].x * X_pcl +(RGBimage.size().width/2);//(RightX)
         CENPX = (pointCloud -> points[*(it->indices.end ()-1)].x+pointCloud -> points[*(it->indices.begin ())].x)/2;//(CenterX)
         CENPXMIN = (pointCloud -> points[*(it->indices.end ()-1)].x+CENPX)/2;//LeftXとCenterXの間のX
         CENPXMAX = (pointCloud -> points[*(it->indices.begin ())].x+CENPX)/2;//RightXとCenterXの間のX
     ...
     for (std::vector<int>::const_iterator pit = it->indices.begin (); pit != it->indices.end (); ++pit){ // 中心点にアクセスするループ(pit=グループ内の点番号
        ...
        if(cenpx>=std::abs(CENPX-pointCloud -> points[*pit].x)){cenpx = std::abs(CENPX-pointCloud -> points[*pit].x);//大枠の中心点のX座標を求める
            centerx = (pointCloud -> points[*pit].x * X_pcl +(RGBimage.size().width/2));//X_Y_wariaiでかけるとピクセル値になる
        ...
        if(cenpxmin>=std::abs(CENPXMIN-pointCloud -> points[*pit].x)){cenpxmin = std::abs(CENPXMIN-pointCloud -> points[*pit].x);//大枠の中心点とLeftXの間のX座標を求める
            centerleftx = (pointCloud -> points[*pit].x * X_pcl +(RGBimage.size().width/2));//X_Y_wariaiでかけるとピクセル値になる
            centerleftz = pointCloud -> points[*pit].z;
            }//if文終了

        if(cenpxmax>=std::abs(CENPXMAX-pointCloud -> points[*pit].x)){cenpx = std::abs(CENPXMAX-pointCloud -> points[*pit].x);//大枠の中心点とRightXの間のX座標を求める
            centerrightx = (pointCloud -> points[*pit].x * X_pcl +(RGBimage.size().width/2));//X_Y_wariaiでかけるとピクセル値になる
    ...
    OpenCV1::waku waku;
    waku.point_left.x = (MINPX-(RGBimage.size().width/2))/X_pcl;//送るデータ作成(大枠の右下と左下の座標)
    waku.point_left.y = (MAXPY-(RGBimage.size().height/2))/Y_pcl;
