...
std::cout<<"test"<<std::endl;
    ...
    std::vector<std::vector<double>> B(RGBimage.size().width, std::vector<double>(RGBimage.size().height));
...
std::cout <<"["<<x<<"]["<<y<<"]="<<depthimage.at<float>(y,x)<< std::endl;//printf
        ...
        for(int y = 0;y < RGBimage.size().height-H;y=y+H){
            ...
            siro=0,kaisu=0,P1=0;
                    ...
                    cutdepth_img.forEach<float>([&](float &p, const int  position[]) -> void{
                        ...
                        if(kaisu==100){B[x][y]=(depth-(Dmax+Dmin))/(W*H-2);}//最大値と最小値を除いて平均化
   ...
   for(int x=0;x<RGBimage.size().width;x=x+W){
	 for(int y=0;y<RGBimage.size().height;y=y+H){
         //std::cout <<"A["<<x<<"]["<<y<<"]="<<A[x][y]<< std::endl;//printf
         P1=0;
         Q[x][y]=B[x+1][y+1]-B[x][y];
         fabs(Q[x][y]);
         if(Q[x][y]<=1000){P1=1,B[x+1][y+1]=B[x][y];}//B+1とBで比較し少し一致してたら発動
		 if(A[x][y]>=10){                                           //白の割合が10%以上なら発動
             rectangle(img_dst8,Rect(x,y,W,H),Scalar(0,0,255),1);//赤四角作成
             rectangle(img_dst9,Rect(x,y,W,H),Scalar(0,0,255),1);//赤四角作成
             if(P1==1){rectangle(img_dst9,Rect(x,y,W,H),Scalar(255,250,0),1);}//水色四角
             //kyori=B[x][y];
             //putText(img_dst8,kyori,Point(x,y),0,1,Scalar(255,255,0),1);
             //std::cout <<"B["<<x<<"]["<<y<<"]="<<B[x][y]<<"_m"<< std::endl;}
         }
     }}
