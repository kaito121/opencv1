...
string file_cont = "image2/"+file+"/DSC_"+name+"contup.jpg";//コントラスト
string file_meido = "image2/"+file+"/DSC_"+name+"contup_meidoup.jpg";//コントラスト
string file_dst = "image2/"+file+"/DSC_"+name+"contup_meidoup_after2.jpg";//二値化
string file_dst2 = "image2/"+file+"/DSC_"+name+"contup_meidoup_after2_2tika.jpg";//二値化エッジ抽出画像
string file_dst3= "image2/"+file+"/DSC_"+name+"contup_meidoup_after2_2tika_mask.jpg";//mask画像
string file_dst4 = "image2/"+file+"/DSC_"+name+"contup_meidoup_after2_2tika_out.jpg";//カラーフィルタ処理画像
string file_dst5 = "image2/"+file+"/DSC_"+name+"contup_meidoup_after2_2tika_out_UP.jpg";//膨張処理画像
string file_dst6 = "image2/"+file+"/DSC_"+name+"contup_meidoup_after2_2tika_out_UP_mono.jpg";//密度検出画像
string file_dst7 = "image2/"+file+"/DSC_"+name+"contup_meidoup_after2_2tika_out_UP_mono_mitudo.jpg";//密度検出画像
...
//グレースケール化
cvtColor(img_cont,img_src1,COLOR_BGR2GRAY);
//二値化
int thresh =100;
threshold(img_src1,img_dst,thresh,255,THRESH_BINARY);
...
// inRangeを用いてフィルタリング
	...
	//膨張処理
	dilate(img_dst2,img_dst5,element8,Point(-1,-1),1);
...
//結果表示
   for(i=0;i<img_src.size().width;i=i+W){
	 for(j=0;j<img_src.size().height;j=j+H){
		 if(A[i][j]>=30){rectangle(img_dst7,Rect(i,j,W,H),Scalar(0,0,255),2);}//赤四角作成
