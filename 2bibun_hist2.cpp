...
int main()
{
string name = "IMG_0298";
string file = "hist/0298";
string file_src = "image3/"+name+".jpg";//元画像
string file_grey = "image3/"+file+"/B5_"+name+"grey.jpg";//グレースケール
string file_hist = "image3/"+file+"/B5_"+name+"hist.jpg";//ヒストグラム均一化
string file_cont = "image3/"+file+"/B5_"+name+"hist_cont.jpg";//コントラスト
//string file_meido = "image2/"+file+"/DSC_"+name+"hist_contup_meido0.jpg";//明度調整
string file_dst = "image3/"+file+"/B5_"+name+"hist_cont_after2.jpg";//二値化
string file_dst2 = "image3/"+file+"/B5_"+name+"hist_cont_after2_2tika.jpg";//二値化エッジ抽出画像
string file_dst3= "image3/"+file+"/B5_"+name+"hist_cont_after2_2tika_mask.jpg";//mask画像
string file_dst4 = "image3/"+file+"/B5_"+name+"hist_cont_after2_2tika_out.jpg";//カラーフィルタ処理画像
string file_dst5 = "image3/"+file+"/B5_"+name+"hist_cont_after2_2tika_out_UP.jpg";//膨張処理画像
string file_dst6 = "image3/"+file+"/B5_"+name+"hist_cont_after2_2tika_out_UP_mono.jpg";//密度検出画像
string file_dst7 = "image3/"+file+"/B5_"+name+"hist_cont_after2_2tika_out_UP_mono_mitudo.jpg";//密度検出画像
...
std::cout<<"test1"<<std::endl;
   ...
   for(i=0;i<=img_src.size().width-W;i=i+W){
	 for(j=0;j<=img_src.size().height-H;j=j+H){ 
		 ...
		 for(x=i;x<=i+W;x=x+2){
			 for(y=j;y<=j+H;y=y+10){
                 if(x!=0&&y!=0){
                unsigned char intensity = img_dst5.at<unsigned char>(y-8, x);//白か黒かを調べる
	            std::cout <<"2i:"<<i<<"j:"<<j<<"X:"<<x<<"y:"<<y-8<<"a:"<<(unsigned int)intensity<< std::endl;//表示
	            if(intensity>=200){siro=siro+1;}}

                if(x!=0&&y!=0){
	            unsigned char intensity = img_dst5.at<unsigned char>(y-6, x);//白か黒かを調べる
                std::cout <<"2i:"<<i<<"j:"<<j<<"X:"<<x<<"y:"<<y-6<<"a:"<<(unsigned int)intensity<< std::endl;//表示
                if(intensity>=200){siro=siro+1;}}

                if(x!=0&&y!=0){
                unsigned char intensity = img_dst5.at<unsigned char>(y-4, x);//白か黒かを調べる
                std::cout <<"2i:"<<i<<"j:"<<j<<"X:"<<x<<"y:"<<y-4<<"a:"<<(unsigned int)intensity<< std::endl;//表示
                if(intensity>=200){siro=siro+1;}}

                if(x!=0&&y!=0){
                unsigned char intensity = img_dst5.at<unsigned char>(y-2, x);//白か黒かを調べる
                std::cout <<"2i:"<<i<<"j:"<<j<<"X:"<<x<<"y:"<<y-2<<"a:"<<(unsigned int)intensity<< std::endl;//表示
                if(intensity>=200){siro=siro+1;}}

                unsigned char intensity = img_dst5.at<unsigned char>(y, x);//白か黒かを調べる
                std::cout <<"2i:"<<i<<"j:"<<j<<"X:"<<x<<"y:"<<y<<"a:"<<(unsigned int)intensity<< std::endl;//表示
                if(intensity>=200){siro=siro+1;}
...
//結果表示
   for(i=0;i<img_src.size().width;i=i+W){
	 for(j=0;j<img_src.size().height;j=j+H){
		 if(A[i][j]>=25){rectangle(img_dst7,Rect(i,j,W,H),Scalar(255,0,0),2);}//赤四角作成

		 std::cout <<A[i][j]<< std::endl;//printf
     }}

//スキャン２回め
int W2=30,H2=30; siro=0;
   for(i=0;i<=img_src.size().width-W2;i=i+W2){
	 for(j=0;j<=img_src.size().height-H2;j=j+H2){ 
		 siro=0; std::cout<<"test2"<<std::endl;
		 for(x=i;x<=i+W2;x=x+6){
			 for(y=j;y<=j+H2;y=y+6){
	           if(x!=0&&y!=0){
                unsigned char intensity = img_dst5.at<unsigned char>(y-3, x-3);//白か黒かを調べる
...
//結果表示
   for(i=0;i<img_src.size().width;i=i+W2){
	 for(j=0;j<img_src.size().height;j=j+H2){
		 if(A[i][j]>=25){rectangle(img_dst7,Rect(i,j,W2,H2),Scalar(0,0,255),2);}//青四角作成
